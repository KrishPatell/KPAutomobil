...
export default function StepQuote({ state, next, goTo }: StepProps) {
  ...
  const rows: {
    label: string
    value: string
    step: Parameters<StepProps["goTo"]>[0]
  }[] = [
    { label: bookFlow.quote.vehicleLabel, value: sizeName, step: "size" },
    {
      label: bookFlow.quote.packageLabel,
      value: state.service ?? "—",
      step: "package",
    },
    {
    ...
    },
    {
      ...
      value:
        [readable(state.date), state.window].filter(Boolean).join(" · ") || "—",
      step: "contact",
    },
    {
    ...
    <section className="kp-step">
      ...
      <div className="kp-total">
        ...
        <p className="kp-total__figure">
          ...
          <b>
            {result.total === null ? pricing.pendingTotal : money(result.total)}
          </b>
        </p>
        {result.total === null && (
          <p className="kp-total__note">{pricing.pendingNote}</p>
        )}
      </div>
