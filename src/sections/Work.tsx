...
function Card({
  ...
  return (
    <li
      className="kp-work__item kp-reveal"
      ref={revealRef}
      style={revealDelay(index, 60)}
    >
      <button
        className="kp-work__card"
        onClick={() => onBook(shot.packageTitle)}
        type="button"
      >
        ...
        <span className="kp-work__bar">
          ...
          <span aria-hidden="true" className="kp-work__go">
            {work.cta} →
          </span>
        </span>
      </button>
...
export default function Work({ onBook }: WorkProps) {
  ...
  function nudge(direction: -1 | 1) {
    ...
    const step = card
      ? card.getBoundingClientRect().width + 21.6
    ...
    >
      <div className="kp-work__head kp-reveal" ref={headRef}>
        <div className="kp-work__intro">
          ...
          <p className="kp-work__lede kp-reveal" style={revealDelay(2)}>
            {work.intro}
          </p>
        </div>
      ...
      {anyReference && (
        <p className="kp-work__reference">{work.referenceNote}</p>
      )}
    </section>
  )
}
