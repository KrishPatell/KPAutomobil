...
export default function StepPhotos({ state, set, next, goTo }: StepProps) {
  ...
  const stale = PHOTO_SLOTS.some(
    (slot) => state.photos[slot.id] && !state.photos[slot.id]?.file,
  )
  ...
  return (
    <section className="kp-step">
          ...
          return (
            <li className={`kp-shot${entry ? " is-filled" : ""}`} key={slot.id}>
              <div className="kp-shot__frame">
                {entry ? (
                  ...
                  <svg aria-hidden="true" viewBox="0 0 24 24" fill="none">
                    <path
                      d="M4 8.5A1.5 1.5 0 0 1 5.5 7h2L9 5h6l1.5 2h2A1.5 1.5 0 0 1 20 8.5v9a1.5 1.5 0 0 1-1.5 1.5h-13A1.5 1.5 0 0 1 4 17.5v-9Z"
                      stroke="currentColor"
                      strokeWidth="1.6"
                    />
                    <circle
              ...
              <div className="kp-shot__meta">
                ...
                {errors[slot.id] && (
                  <em className="kp-shot__error">{errors[slot.id]}</em>
                )}
      ...
      <div className="kp-step__actions">
        ...
        <KpButton
          disabled={added < PHOTO_SLOTS.length}
          onClick={next}
          size="md"
        >
          {bookFlow.next}
