  ...
  return (
    <span aria-hidden="true" className="kp-size__pips">
      {[1, 2, 3, 4].map((step) => (
        <span
          className={
            step <= filled ? "kp-size__pip kp-size__pip--on" : "kp-size__pip"
          }
          key={step}
        />
      ))}
...
export default function Vehicles({ onSelectSize }: VehiclesProps) {
  ...
  return (
    <section
      ...
      className="kp-section kp-vehicles"
      id="vehicles"
      ref={revealRef}
    >
      <div className="kp-section__inner">
        <div className="kp-vehicles__head">
          ...
          <p className="kp-vehicles__intro kp-reveal" style={revealDelay(2)}>
            {vehicles.intro}
          </p>
        </div>
        ...
        <div
          className="kp-vehicles__rail-head kp-reveal"
          style={revealDelay(7)}
        >
        ...
        <ul className="kp-vehicles__rail kp-reveal" style={revealDelay(8)}>
          {bodyStyles.map((style) => (
            <li key={style.name}>
              <button
                className="kp-body"
                onClick={() => onSelectSize(style.size)}
                type="button"
              >
                <img
                  alt={style.imageAlt}
                  className="kp-body__img"
                  loading="lazy"
                  src={style.image}
                />
                ...
                <span className="kp-body__size">
                  {vehicles.pricedAs}{" "}
                  {sizes.find((s) => s.id === style.size)?.name}
                </span>
              </button>
            </li>
          ))}
